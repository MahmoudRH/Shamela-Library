...
class FolioActivity : AppCompatActivity(), FolioActivityCallback {
    ...
    private lateinit var appBarLayout: FolioAppBarLayout
    private lateinit var toolbar: Toolbar
    private lateinit var handler: Handler
    private var distractionFreeMode: Boolean = false

    override var currentChapterIndex: Int = 0
    private lateinit var mFolioPageFragmentAdapter: FolioPageFragmentAdapter
    override var entryReadLocator: ReadLocator = ReadLocator()
    private var lastReadLocator: ReadLocator = ReadLocator()
    private var outState: Bundle? = null
    private var savedInstanceState: Bundle? = null

    private lateinit var r2StreamerServer: Server
    private lateinit var pubBox: PubBox
    private var spine: List<Link> = listOf()
    ...
    private val currentFragment: FolioPageFragment
        get() = mFolioPageFragmentAdapter.getItem(mFolioPageViewPager.currentItem) as FolioPageFragment

    private enum class RequestCode constructor(val value: Int) {
    ...
    override fun onOptionsItemSelected(item: MenuItem): Boolean {
        ...
        when (item.itemId) {
            ...
            R.id.itemSearch -> {
                ...
                intent.putExtra(SearchActivity.BUNDLE_SPINE_SIZE, spine.size)
...
//        val tableOfContents = pubBox.publication.tableOfContents.ifEmpty { pubBox.publication.readingOrder }
    ...
    private fun initBook() {
        ...
        bookFileName = FileUtil.getEpubFilename(mEpubFilePath)
        Log.v(LOG_TAG, "-> bookFileName= $bookFileName")
        EpubParser().parse(mEpubFilePath, "")?.let {
            pubBox = it
        }
        portNumber =
...
/**
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        ...
        if (requestCode == RequestCode.SEARCH.value) {
            ...
            if (resultCode == SearchActivity.ResultCode.ITEM_SELECTED.value) {
                ...
                searchLocator = if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.TIRAMISU) {
                ...
                currentChapterIndex = getChapterIndex( searchLocator!!.href)
                mFolioPageViewPager.currentItem = currentChapterIndex
                val folioPageFragment = currentFragment
            ...
            if (type == CHAPTER_SELECTED) {
                ...
                val highlightImpl = if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.TIRAMISU) {
                    data.getParcelableExtra(HIGHLIGHT_ITEM, HighlightImpl::class.java)
                } else {
                    data.getParcelableExtra(HIGHLIGHT_ITEM)
                }
                highlightImpl?.let {
                    currentChapterIndex = it.pageNumber
                    mFolioPageViewPager.currentItem = currentChapterIndex
                    val folioPageFragment = currentFragment
                    it.rangy.let { it1 -> folioPageFragment.scrollToHighlightId(it1) }
                }
            }
        }
    }
    ...
    private fun configFolio() {
        ...
        if (searchLocator != null) {
            ...
            if (savedInstanceState == null) {
                readLocator = if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.TIRAMISU) {
                    intent.getParcelableExtra(
                        EXTRA_READ_LOCATOR,
                        ReadLocator::class.java
                    )
                } else {
                    intent.getParcelableExtra(EXTRA_READ_LOCATOR)
                }
                entryReadLocator = readLocator ?: ReadLocator()
            } else {
                readLocator = savedInstanceState!!.getParcelable(BUNDLE_READ_LOCATOR_CONFIG_CHANGE)
                lastReadLocator = readLocator ?: ReadLocator()
            }
    ...
    private fun getChapterIndex(readLocator: ReadLocator?): Int {
        ...
        if (readLocator == null) {
            ...
            return getChapterIndex(readLocator.href)
        }

        return 0
    }

    private fun getChapterIndex(value: String): Int {
        for (i in spine.indices) {
            if (spine[i].href == value)  return i
        }
        return 0
    }
...
/**
    ...
    private fun setConfig(savedInstanceState: Bundle?) {
        ...
        val config: Config
        val intentConfig = if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.TIRAMISU) {
            intent.getParcelableExtra(Config.INTENT_CONFIG, Config::class.java)
        } else {
            intent.getParcelableExtra(Config.INTENT_CONFIG)
        }
        val overrideConfig = intent.getBooleanExtra(Config.EXTRA_OVERRIDE_CONFIG, false)
        val savedConfig = AppUtil.getSavedConfig(this)

        config = if (savedInstanceState != null && savedConfig != null) {
            savedConfig
